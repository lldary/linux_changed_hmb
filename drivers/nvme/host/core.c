...
static inline enum nvme_disposition nvme_decide_disposition(struct request *req)
{
	...
	if (blk_noretry_request(req) || (nvme_req(req)->status & NVME_SC_DNR) ||
...
void *nvme_buf_list_to_prp(struct nvme_command *cmd, MCPEntry *head)
{
	...
	int will_copy_ = 0;
	printk(KERN_ERR
	       "nvme_buf_list_to_prp: headptr: %llx blocksize: %lu total_size: %llu ori_block: %lu ori_data: %lu mcp_count: %u",
	       head, block_size, le64_to_cpu(head->total_size),
	       head->block_size, head->total_size, mcp_count);
	// head->block_size = prp1;
	// head->total_size = prp2;
	if (block_size > 8192) {
		printk(KERN_CRIT
		       "nvme_buf_list_to_prp: headptr: %llx blocksize: %lu total_len: %llu ori_block: %lu ori_data: %lu",
	...
	while (len > 0) {
		if (will_copy_) {
			size_t chunk_size = min(remain_page_size, block_size);
			printk(KERN_ERR
			       "nvme_buf_list_to_prp: from %lx to %lx size: %u mcp_count: %u remain_lenght: %lu",
			       le64_to_cpu(cur->v_addr), vaddr + offset,
			       chunk_size, mcp_count, len);
		...
		if (remaining_len == 0 && mcp_count != 1) {
			if (prp_entry_size - offset >= block_size) {
				printk(KERN_ERR
				       "nvme_buf_list_to_prp: from %lx to %lx size: %x mcp_count: %u remain_lenght: %lu",
				       le64_to_cpu(cur->v_addr), vaddr + offset,
				       block_size, mcp_count, len);
				memmove(vaddr + offset,
					le64_to_cpu(cur->v_addr), block_size);
				offset += block_size;
				remain_page_size -= block_size;
				len -= block_size;
			} else
				will_copy_ = 1;
...
static void nvme_execute_rq_polled(struct request_queue *q,
				   struct gendisk *bd_disk, struct request *rq,
				   int at_head)
{
...
int __nvme_submit_sync_cmd(struct request_queue *q, struct nvme_command *cmd,
			   ...
			   unsigned bufflen, unsigned timeout, int qid,
			   int at_head, blk_mq_req_flags_t flags, bool poll)
{
	struct request *req;
	int ret;
...
static void nvme_update_disk_info(struct gendisk *disk, struct nvme_ns *ns,
...
static int nvme_pr_command(struct block_device *bdev, u32 cdw10, u64 key,
			   u64 sa_key, u8 op)
{
...
static int nvme_pr_register(struct block_device *bdev, u64 old, u64 new,
			    unsigned flags)
{
	u32 cdw10;
					    ...
					    struct attribute *a, int n)
{
	...
	if (a == &dev_attr_ana_grpid.attr || a == &dev_attr_ana_state.attr) {
		if (dev_to_disk(dev)->fops !=
		    &nvme_bdev_ops) /* per-path attr */
			return 0;
...
static struct attribute *nvme_dev_attrs[] = { &dev_attr_reset_controller.attr,
					       ...
					       struct nvme_ns_ids *ids)
{
	...
	if (ret) {
		dev_err(ctrl->device, "duplicate IDs for nsid %d\n", nsid);
					   ...
					   unsigned nsid)
{
	...
	list_for_each_entry_safe (ns, next, &ctrl->namespaces, list) {
		if (ns->head->ns_id > nsid ||
		    test_bit(NVME_NS_DEAD, &ns->flags))
...
static void nvme_fw_act_work(struct work_struct *work)
{
	...
	if (ctrl->mtfa)
		...
		fw_act_timeout =
			jiffies + msecs_to_jiffies(admin_timeout * 1000);
...
static int __init nvme_core_init(void)
{
	...
	nvme_reset_wq = alloc_workqueue(
		"nvme-reset-wq", WQ_UNBOUND | WQ_MEM_RECLAIM | WQ_SYSFS, 0);
	if (!nvme_reset_wq)
		goto destroy_wq;

	nvme_delete_wq = alloc_workqueue(
		"nvme-delete-wq", WQ_UNBOUND | WQ_MEM_RECLAIM | WQ_SYSFS, 0);
	if (!nvme_delete_wq)
